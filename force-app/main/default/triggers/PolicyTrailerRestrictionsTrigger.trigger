trigger PolicyTrailerRestrictionsTrigger on Policy_Trailer__c (before insert, before update, before delete) {
    ...
    // Collect Policy_Draft__c IDs from Policy_Trailer__c records in the trigger context
    Set<Id> policyDraftIds = new Set<Id>();
    if (Trigger.isInsert || Trigger.isUpdate || Trigger.isDelete) {
        for (Policy_Trailer__c record : (Trigger.isInsert ? Trigger.new : Trigger.old)) {
    ...
    // Validate insertions
    if (Trigger.isInsert) {
        for (Policy_Trailer__c record : Trigger.new) {
            if (record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot create a Policy Trailer record because the related Policy Draft is in "Ready to Bind" status.');
            }
        }
    }
    ...
    if (Trigger.isUpdate) {
        for (Policy_Trailer__c record : Trigger.new) {
            Policy_Trailer__c oldRecord = Trigger.oldMap.get(record.Id);

            // Restrict updates if the related Policy_Draft__c.Status__c is "Ready to Bind"
            if (record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot update this Policy Trailer record because the related Policy Draft is in "Ready to Bind" status.');
            }
    ...
    // Validate deletions
    if (Trigger.isDelete) {
        for (Policy_Trailer__c record : Trigger.old) {
            // Restrict deletions if the related Policy_Draft__c.Status__c is "Ready to Bind"
            if (record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot delete this Policy Trailer record because the related Policy Draft is in "Ready to Bind" status.');
            }
