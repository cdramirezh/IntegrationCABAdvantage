trigger CoveragesRestrictionsTrigger on Coverage__c (before insert, before update, before delete) {
    ...
    // Collect Policy_Draft__c IDs from Coverage__c records in the trigger context
    Set<Id> policyDraftIds = new Set<Id>();
    if (Trigger.isInsert || Trigger.isUpdate || Trigger.isDelete) {
        for (Coverage__c record : (Trigger.isInsert ? Trigger.new : Trigger.old)) {
    ...
    // Validate insertions
    if (Trigger.isInsert) {
        for (Coverage__c record : Trigger.new) {
            if (record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot create a Coverage record because the related Policy Draft is in "Ready to Bind" status.');
            }
        }
    }
    ...
    if (Trigger.isUpdate) {
        for (Coverage__c record : Trigger.new) {
            Coverage__c oldRecord = Trigger.oldMap.get(record.Id);

            // Restrict updates if the related Policy_Draft__c.Status__c is "Ready to Bind"
            if (!isAllowedUser && record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot update this Coverage record because the related Policy Draft is in "Ready to Bind" status.');
            }
    ...
    // Validate deletions
    if (Trigger.isDelete) {
        for (Coverage__c record : Trigger.old) {
            // Restrict deletions if the related Policy_Draft__c.Status__c is "Ready to Bind"
            if (record.Policy_Draft__c != null &&
                policyDraftStatuses.get(record.Policy_Draft__c) == 'Ready to Bind') {
                record.addError('You cannot delete this Coverage record because the related Policy Draft is in "Ready to Bind" status.');
            }
